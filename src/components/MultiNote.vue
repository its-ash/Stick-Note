  ...
  <div>
    <div class="col-md-4 col-sm-6">
      <div class="single-note add-note" v-bind:class="[newNote.color]">
        <label for="dummy-input"></label>
        <input id="dummy-input" type="text" placeholder="Type a title ..." v-model="noteTitle">
        <small>Today: {{ todayDate }}</small>
        <hr>
        <label for="dummy-input-area"></label>
        <textarea id="dummy-input-area" placeholder="Type a description ..." v-model="noteText"></textarea>
        <div class="meta">
        ...
        <div class="colors" :class="{openDivs: currentID === -1}">
          ...
          <div @click="noteColor = 'white'" class="circle whiteCircle"
               :class="{selected: newNote.color === 'white'}"></div>
        </div>
      </div>
    </div>
